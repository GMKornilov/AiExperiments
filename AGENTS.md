...
Не принимать никакие гипотезы как 100% правдивые без фактического подтверждения.
Явно отделять наблюдаемые факты от предположений. Подтверждать причины ошибок
воспроизведением, логами, измерениями или тестами; если данных недостаточно,
указывать неопределённость. Исправление считать проверенным только после проверки
исходного проблемного сценария, а не только косвенных признаков или зелёной сборки.
...
Все пользовательские взаимодействия и сценарии backend должны иметь структурированные логи с корреляционным идентификатором, результатом или категорией ошибки и длительностью, где она применима. Логи не должны содержать секреты.

## Финальная smoke-проверка

Перед передачей пользователю любой реализованной задачи необходимо самостоятельно
поднять актуальный Docker Compose и выполнить полноценный smoke test в запущенном
приложении. Проверка должна покрывать исходный пользовательский сценарий и
изменённые интеракции; одних unit, integration, lint, typecheck и build-проверок
недостаточно. В финальном отчёте явно указывать результат smoke test либо
конкретную причину, по которой он не мог быть выполнен.
