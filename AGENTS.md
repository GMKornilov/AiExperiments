...
## Тесты через реальный LLM API

Если ожидаемое суммарное количество внешних LLM-вызовов в тестовом прогоне не
превышает 100, такой прогон разрешено выполнять без дополнительного
подтверждения пользователя. В оценку включать вспомогательные вызовы, включая
title, summary и memory extractor. Если ожидается более 100 вызовов, перед
запуском запросить у пользователя явное разрешение.
