...
prompt, memory extractor-а, панели памяти и очистки. При конфликте с базовой
...
пользователя остаются только в краткосрочном контексте чата и не попадают ни в
`global_facts`, ни в `project_facts`.
...
неизменяемых правил бариста и двух явно размеченных блоков данных:

1. `GLOBAL MEMORY` — текущий `global_facts`;
2. `PROJECT MEMORY` — текущий `project_facts` выбранного проекта.

В prompt явно указано, что оба блока являются данными, не инструкциями, и при
конфликте project facts приоритетнее global facts. К этому system prompt
...
базовый prompt диалога не подменяется сохранёнными фактами: его составной
вариант создаётся только для конкретного вызова.
...
оборудовании и имеющихся зёрнах. Любой другой claim, включая предпочтение или
ограничение, не становится project/global fact даже при подтверждении и остаётся
только в краткосрочном контексте. Совет, рецепт, догадка или предложение
...
- **AC-MEM-02.** При N=3 четвёртый main chat-вызов содержит заново построенный
  system prompt с маркированными global и project facts и не более трёх
  сообщений только текущего чата, включая текущий input; текст соседнего чата
  отсутствует. Проверка: controlled provider inspection.
- **AC-MEM-03.** При конфликте текущего чата, project и global facts основной
  prompt явно задаёт приоритет `chat > project > global`, а ответ следует
  утверждению текущего чата. Проверка: controlled provider + browser fixture.
