...
основного ответа. Она дополняет [базовую спецификацию](../barista-agent/SPEC.md) и
[спецификацию инвариантов](../invariants/SPEC.md).
...
retry основной беседы и безопасными ошибками. `invariants` владеет проверкой
input/candidate, repair/refusal lifecycle и публичной панелью правил; этот
документ определяет facts, которые такие проверки используют. Исторические решения находятся
...
| Рабочая память | Текущий проект | `project_facts`: подтверждённые пользователем факты, которые он явно ограничил этим проектом. |
| Долговременная память | Весь browser-сеанс | `global_facts`: подтверждённые пользователем факты, полезные проектам этой сессии; это scope по умолчанию для equipment/beans. |
...
extractor может сохранять только пользовательское оборудование и status
доступности оборудования/зёрен. Оборудование может описываться типом, названием
и моделью, зёрна — названием, обжаркой, количеством и датой. Факт «нет»,
«сломано» или «закончилось» — допустимый отрицательный факт доступности, а не
отсутствие записи. Прямо подтверждённые facts об equipment/beans сохраняются в
`global_facts`, если пользователь явно не ограничил их текущим проектом; только
такое явное ограничение допускает `project_facts`. Предпочтения, ограничения и любые иные claims
...
слоя в более узкий. Historical project facts не повышаются автоматически,
поскольку их прежний scope неизвестен. Отсутствие fact означает только
неизвестность ресурса, а не его недоступность.

### Единый цикл ответа и памяти

Для каждого user-input агент проходит цикл, определённый
[инвариантами](../invariants/SPEC.md): pre-validation, generation и
post-validation могут дать final candidate либо template refusal. Ни один
intermediate candidate не считается принятым ответом. Для final accepted pair
один memory extractor обязан завершиться до сохранения и показа ответа. Пока
цикл не завершён, UI показывает общий pending/progress, а не частичный ответ.

```mermaid
flowchart LR
    U[User Input] --> I[Pre-validation инвариантов]
    I -- allow --> L[Load: global и project memory<br/>+ short-term N текущего чата]
    I -- technical error --> F[Safe retry без pair и snapshots]
    L --> P[Build main system prompt]
    P --> C[Main candidate и post-validation/repair]
    I -- violation --> E[Memory extractor<br/>для final user + refusal pair]
    C --> E[Memory extractor<br/>для final user + assistant pair]
    C -- technical error --> F
    E --> V{JSON валиден?}
...
Если generation или validator завершается technical error, extractor не
запускается и сохраняется retry-поведение из `invariants`. При успешном
extractor-е user/assistant pair и оба memory snapshots сохраняются согласованно
до показа ответа. При timeout,
...
неизменяемых правил бариста, системных ограничений из
[спецификации инвариантов](../invariants/SPEC.md), активного профиля из
...
текущего чата и памяти; а project facts приоритетнее global facts. Системные
инварианты имеют приоритет над profile, chat и memory. К этому system prompt
...
оборудовании и status доступности зёрен. Любой другой claim из беседы, включая
...
отсутствует, сломан или закончился, заменяет противоречащий positive fact
отрицательным fact в том же scope; оно не удаляет знание о недоступности.
Если scope не ограничен пользователем явно, такой fact попадает в global
snapshot.
...
extractor-вызова и согласованно сохраняет их с final user/assistant pair,
включая template refusal из `invariants`. При timeout,
...
- **AC-MEM-03.** При конфликте active profile, текущего чата, project и global
  facts основной prompt явно задаёт приоритет `invariants > profile > chat >
  project > global`, а response не нарушает static инвариант и в допустимой
  части следует активному профилю. Проверка: controlled provider + browser
  fixture.
- **AC-MEM-04.** После final accepted candidate или template refusal запускается
  ровно один extractor; при искусственно задержанном extractor-е response не
  отображается, а UI показывает `updating`. После успешного extractor-а и
  сохранения pair response отображается. Technical validator error extractor
  не запускает. Проверка: controlled provider timing + browser.
- **AC-MEM-05.** Extractor, получивший подтверждённый пользователем факт об
  оборудовании или status доступности зёрен, возвращает валидный полный
  snapshot; следующий main chat-вызов использует обновлённые global/project
  facts. Equipment/beans facts по умолчанию global; явное project-ограничение
  сохраняет project scope, а отсутствие/поломка/окончание остаётся отрицательным
  fact. Подтверждённое предпочтение или ограничение, а также предложение
  assistant-а без пользовательского подтверждения, в snapshot не попадают.
  Проверка: integration fixture.
