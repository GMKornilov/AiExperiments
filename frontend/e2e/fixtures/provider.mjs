...
createServer(async (request, response) => {
  ...
  seen.set(text, count);
  if (text.startsWith("slow")) await new Promise((resolve) => setTimeout(resolve, 8000));
  if (text.startsWith("fail") && count === 1) { response.writeHead(503).end("fixture provider failure"); return; }
  const answer = `Ответ: ${text} [${body.model}]`;
